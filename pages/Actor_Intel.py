...
st.set_page_config(page_title="Actor Intel", page_icon="🎭", layout="wide")
...
def get_actor_movies(person_id):
    ...
    cast = data.get("cast", [])
    # filter noise -- only movies with decent vote count
...
def get_badge(hit_rate):
    # fun badge based on hit rate
    if hit_rate >= 75:
        return "💚 GUARANTEED GREEN", "#00ff00"
    elif hit_rate >= 60:
...
# header
st.markdown("<div class='hero-title'>🎭 ACTOR<span class='red-accent'> INTEL</span></div>", unsafe_allow_html=True)
st.markdown("<div style='color:#888; font-size:0.9rem; letter-spacing:3px; text-transform:uppercase; margin-top:0.5rem'>Career hit rate, biggest hits, biggest flops</div>", unsafe_allow_html=True)
st.markdown("<div class='divider'></div>", unsafe_allow_html=True)

actor_name = st.text_input("Search Actor", placeholder="e.g. Tom Hanks, Leonardo DiCaprio...")
search_btn = st.button("SEARCH")

if search_btn and actor_name:
    with st.spinner("Pulling records..."):
        person = search_actor(actor_name)

    if not person:
        st.markdown("<div style='color:#A01830'>Actor not found.</div>", unsafe_allow_html=True)
    else:
        person_id = person["id"]
        movies = get_actor_movies(person_id)

        if not movies:
            st.markdown("<div style='color:#666'>No credits found.</div>", unsafe_allow_html=True)
        else:
            ...
            with left:
                ...
                # badge
                st.markdown(f"<div style='font-size:0.85rem; color:{badge_color}; font-weight:600; margin-top:0.5rem'>{badge}</div>", unsafe_allow_html=True)
            ...
            with right:
                ...
                with s2:
                    st.markdown(f"<div class='stat-card'><div class='stat-number'>{len(movies)}</div><div class='stat-label'>Movies</div></div>", unsafe_allow_html=True)
                with s3:
                    st.markdown(f"<div class='stat-card'><div class='stat-number'>{len(hits)}</div><div class='stat-label'>Hits</div></div>", unsafe_allow_html=True)
                with s4:
                ...
                st.markdown(f"""
                <div class='prob-bar-container' style='height:12px; margin-top:1rem'>
                    <div class='prob-bar-fill' style='width:{hit_rate}%; background:{bar_color}'></div>
                </div>
                <div style='color:#666; font-size:0.7rem; margin-top:0.3rem'>{len(hits)} hits out of {len(movies)} movies</div>
                """, unsafe_allow_html=True)
                ...
                with b1:
                    st.markdown(f"""
                    <div class='stat-card'>
                        <div style='font-size:0.65rem; color:#00ff00; letter-spacing:2px'>BIGGEST HIT</div>
                ...
                with b2:
                    st.markdown(f"""
                    <div class='stat-card'>
                        <div style='font-size:0.65rem; color:#A01830; letter-spacing:2px'>BIGGEST FLOP</div>
